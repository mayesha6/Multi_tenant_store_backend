...
const getAllSubscriptions = async (currentUser) => {
...
const createCheckoutSession = async (currentUser, planId) => {
    if (!currentUser.tenantId) {
        throw new AppError(httpStatus.BAD_REQUEST, "User has no tenant");
    }
    ...
    if (activeSub) {
        throw new AppError(httpStatus.BAD_REQUEST, "Tenant already has an active subscription");
    }
